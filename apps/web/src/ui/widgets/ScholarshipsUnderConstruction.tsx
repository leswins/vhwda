...
export function ScholarshipsUnderConstruction({ language }: Props) {
  const tapeText = `${t(language, "resources.scholarships.underConstruction")} • ${t(language, "resources.scholarships.comingSoon")}`
      ...
      <div className="hidden lg:flex items-center justify-between p-[25px] border-y-[0.5px] border-foreground">
        <div className="flex items-center gap-[20px]">
          <div className="flex h-[70px] w-[70px] items-center justify-center bg-[rgb(var(--color-accent-green))]">
            ...
            <svg width="35" height="35" viewBox="0 0 50 50" fill="none" aria-hidden="true" className="text-foreground">
          ...
          <h2 className="text-h3 font-bold text-foreground">{t(language, "planNextSteps.card.scholarships.title")}</h2>
        </div>
      </div>

      {/* Body: fixed height to preserve layout */}
      <div className="relative h-[220px] lg:h-[400px] bg-surface">
        {/* Tape */}
        <div className="absolute left-1/2 top-1/2 w-[140%] -translate-x-1/2 -translate-y-1/2 rotate-6 bg-accentYellow border-y border-foreground py-3 lg:py-4">
          <div className="flex items-center gap-6 text-foreground whitespace-nowrap">
            {tapeRepeats.map((text, idx) => (
              <span key={idx} className="text-sub2 font-bold uppercase tracking-[0.15em]">
                {text}
              </span>
            ))}
          </div>
